...
DEFAULT_USER_AGENT = "aholo-sdk-core/1.2.0"
...
class AsyncAholoGatewayClient(AsyncBaseHttpClient):
    def __init__(self, config: Optional[AholoClientConfig] = None) -> None:
        config = config or AholoClientConfig()
        api_key = resolve_api_key(config)
        user_agent = f"{DEFAULT_USER_AGENT} {config.user_agent}" if config.user_agent else DEFAULT_USER_AGENT
    ...
    async def gateway_request(
        ...
        *,
        ...
        return await self.request(
            method=method, path=path, query=query, json_body=body, headers=headers, timeout_ms=timeout_ms
        )


def create_async_gateway_client(config: Optional[AholoClientConfig] = None) -> AsyncAholoGatewayClient:
    return AsyncAholoGatewayClient(config)
